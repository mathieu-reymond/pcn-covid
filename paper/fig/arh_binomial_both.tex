\begin{tikzpicture}
    ...
    \begin{groupplot}[
        ...
        legend entries={{$\text{PCN}_{b=2}$}, {$\text{PCN}_{b=3}$}, {$\text{PCN}_{b=4}$}, {$\text{PCN}_{b=5}$}, {$\text{PCN}_{b=\infty}$}, {Fixed}},
    ]
    % interpolated plot
    ...
    {
        \edef\temp{
            \noexpand\addplot[only marks,mark=o,fill opacity=0.5,\c] table [x expr=-1*\noexpand\thisrow{o_1}, y expr=-1*\noexpand\thisrow{o_5}, col sep=comma] {data/cs/arh/binomial/budget_\b.csv};
        }
        \temp
    }
    \addplot[only marks,mark=+] table [x expr=-1*\thisrow{o_0}, y expr=-1*\thisrow{o_1}, col sep=comma] {data/cs/cs_fixed.csv};
