\begin{tikzpicture}
    ...
    % interpolated plot for MONES
    \nextgroupplot[mark size=1pt,draw opacity=1,legend entries={{$\text{MONES}_{b=2}$}, {$\text{MONES}_{b=3}$}, {$\text{MONES}_{b=4}$}, {$\text{MONES}_{b=5}$}, {$\text{MONES}$}, {Fixed}},title={MONES algorithm, different budgets}]
    % mean line
    \foreach \b/\c in {2/pc1, 3/pc2, 4/pc3, 5/pc4, {None}/pc5}
    {
        \edef\temp{
            \noexpand\addplot[fill opacity=0.5,\c] table [x expr=-1*\noexpand\thisrow{mu_o_1}, y expr=-1*\noexpand\thisrow{mu_o_5}, col sep=comma] {data/mones/arh/ode/interpolated/budget_\b.csv};
        }
        \temp
    }
    ...
    {
        \edef\temp{
            \noexpand\addplot[name path=upper\b, draw=none] table [x expr=-1*\noexpand\thisrow{mu_o_1}, y expr=-1*\noexpand\thisrow{mu_o_5}+\noexpand\thisrow{std_o_5}, col sep=comma] {data/mones/arh/ode/interpolated/budget_\b.csv};
    ...
    \pgfplotsforeachungrouped \b in {2, 3, 4, 5, {None}}
    {
        % mean plots
        \edef\temp{
            \noexpand\nextgroupplot[draw opacity=1,legend entries={$\text{PCN}_{b=\b}$, $\text{MONES}_{b=\b}$},title={budget $\b$}]
            \noexpand\addplot[fill opacity=0.5,pc1] table [x expr=-1*\noexpand\thisrow{mu_o_1}, y expr=-1*\noexpand\thisrow{mu_o_5}, col sep=comma] {data/cs/arh/ode/interpolated/budget_\b.csv};
