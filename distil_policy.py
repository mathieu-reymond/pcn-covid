    ...
    for p_i, policy_runs in enumerate(transitions):
        ...
        for run in policy_runs:
            for i, transition in enumerate(run):
                ...
                x_t = np.concatenate((transition.observation[STATE_IDX].flatten(), transition.observation[EVENT_IDX], transition.observation[BUDGET_IDX]))
...
def _train_sdt(x, y, policy_specific):
    ...
    depth = 2 if policy_specific else 5
...
def visualize_sdt2(sdt, transitions=None, policy_i=None, with_targets=False, with_aggregate_compartments=False):
    ...
    for depth in range(depths):
        for i in range(2**depth):
            ...
            budget = w[131:134]
            budgets = np.full(compartments.shape[0], np.nan)
            budgets[:3] = budget
            # if with_targets:
            #     t_r = w[131:133]
            #     t_h = w[133:]
            #     extras[5:7] = t_r
            #     extras[9] = t_h
            compartments = np.concatenate((compartments, extras[:, None], budgets[:, None]), 1)
            # get figure position
    ...
    if transitions is not None:
        ...
        for t_i, transition in enumerate(transitions):
            ...
            states = np.array([t.observation[STATE_IDX] for t in transitions])
            # add final state
            states = np.concatenate((states, transitions[-1].next_observation[STATE_IDX][None]), axis=0)
            ...
            obs = np.concatenate((obs[STATE_IDX].flatten(), obs[EVENT_IDX].flatten(), obs[BUDGET_IDX].flatten()))
            if with_targets:
...
def predict(sdt, observation, desired_return, desired_horizon, policy_i=None, with_targets=False):
    compartments = observation[STATE_IDX].flatten()
    school_holidays = observation[EVENT_IDX].flatten()
    budget = observation[BUDGET_IDX].flatten()
    observation = np.concatenate((compartments, school_holidays ,budget))
    if with_targets:
        if policy_i is not None:
...
def make_env(config, scale):
    ...
    budget = '' if config['budget'] is None else f'Budget{config["budget"]}'
    breakpoint()
    ...
    for e_type in ('ODE', 'Binomial'):
        env = gym.make(f'BECovidWithLockdown{e_type}{budget}Continuous-v0')
